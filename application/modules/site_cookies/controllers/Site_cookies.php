...
function test()
{
	...
	echo "<hr>";
	echo anchor('site_cookies/test_unset', 'delete the cookie');
...
function test_unset()
{
	...
	echo "the cookie has now been destroyed<br>";
...
function _set_cookie($user_id)
{
	...
	$data['cookie_code'] = $this->site_security->_generate_random_string(128);
...
{
	//check to see if the user has a valid cookie and if so , figure out the userid
	$this->load->module('site_settings');
	$cookie_name = $this->site_settings->_get_cookie_name();
	...
	if(isset($_COOKIE[$cookie_name]))
	{
		...
		//they have the cookie, but is it still in the table?
